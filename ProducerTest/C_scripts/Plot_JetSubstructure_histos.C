...
void Plot_JetSubstructure_histos()
{
	...
	for (int NoFile=0; NoFile<NoFiles; NoFile++)
	{
		...
		{ 
			...
			h_genJet_pt[NoFile][h] = (TH1D*)(f_input->Get(name));

			sprintf(name,"h_genJet_mass_%s_bin%i",legend_array[NoFile],h);
			h_genJet_mass[NoFile][h] = (TH1D*)(f_input->Get(name));
...
//======================================= create plots ===============================
	...
	for (int NoFile=0; NoFile<NoFiles; NoFile++)
	{
		...
		for(int iy=0; iy<eta_bins; iy++)
		{
			...
			if (NoFile==0 )  framesoftDrop_m[iy] = InitiateFrameOnCanvasPad(pad_softDrop_m, iy+1, "framesoftDrop_m", "Jet SoftDrop mass (GeV) ", "Entries", 0., 200 , 0.8, 1.3*h_ak8_softDrop_m[NoFile][iy]->GetMaximum() , false, paveCMS);
			...
			if (NoFile==0 )  frametau21[iy] = InitiateFrameOnCanvasPad(pad_tau21, iy+1, "frametau21", "#tau_{2} / #tau_{1} ", "Entries", 0., 1.2 , YaxisLowEndMultiplier*h_ak8_tau21[NoFile][iy]->GetMaximum(), YaxisHighEndMultiplier*h_ak8_tau21[NoFile][iy]->GetMaximum() , true, paveCMS);
